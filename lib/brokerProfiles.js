import { inferCategory } from "./portfolioParser.js";
...
function mergeAliases(profile) {
  return Object.fromEntries(
    Object.entries(BASE_ALIASES).map(([field, aliases]) => [
      field,
      [...new Set([...(profile?.aliases?.[field] || []), ...aliases].map(normalized))],
    ]),
  );
}

function detectBroker({ text = "", fileName = "", headers = [] } = {}) {
  const evidence = `${fileName}\n${text}\n${headers.join(" ")}`.slice(0, 20000);
  let best = null;
  ...
  if (!best?.matches) {
    return {
      ...
      confidence: "low",
      message: "Broker not recognized - using generic import.",
  ...
  return {
    ...
    message: `Detected: ${best.profile.label} (confidence: ${best.matches >= 2 ? "high" : "medium"})`,
...
function normalizeBrokerRows(rows = [], detection, sourceRef = "Imported file") {
  ...
  return rows.flatMap((row, index) => {
    ...
    if ((!symbol && !name) || (marketValue === null && weight === null)) return [];
    const assetClass = inferCategory(symbol, name);
    const uncertain = !symbol || assetClass === "Other";
    const warnings = uncertain
      ? [{
          code: !symbol ? "missing-symbol" : "unknown-classification",
          message: !symbol ? `Row ${index + 1} has no recognized symbol.` : `${symbol} needs a confirmed category.`,
          action: "Review and correct this holding before analysis.",
          severity: "warning",
        }]
      : [];
    return [{
      ...
      percent: weight,
      weight,
      category: assetClass === "Other" ? "Needs review" : assetClass,
      assetClass: assetClass === "Other" ? "Needs review" : assetClass,
      confidence: uncertain ? "low" : detection.confidence,
      sourceRef,
      warnings,
    }];
  });
