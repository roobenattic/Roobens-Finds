...
function detectBroker({ text = "", fileName = "", headers = [] } = {}) {
  ...
  if (!best?.matches) {
    return {
      ...
      confidence: "low",
      message: "We could not identify the statement provider, so we used a general layout.",
  ...
  return {
    ...
    message: `We recognized this as a ${best.profile.label} statement and matched its familiar layout.`,
