...
const BLOCKED_TICKERS = new Set([
  ...
  "MONEY", "FUND", "FUNDS", "STOCK", "DIV", "DIVIDEND", "YLD", "ETN", "ETNS",
]);
...
function fidelityNameFromRow(line = "", ticker = "") {
  const beforeTicker = String(line).split(`(${ticker})`)[0];
  return beforeTicker
    .replace(/-?\$?\d[\d,]*(?:\.\d+)?[\s\S]*$/, "")
    .replace(/\b(?:ISIN|SEDOL|CUSIP)\b[\s\S]*$/i, "")
    .trim();
}
...
function parseFidelityStatement(lines) {
  ...
  lines.forEach((line, index) => {
    ...
    if (!ticker || BLOCKED_TICKERS.has(ticker) || seen.has(ticker)) return;
    ...
    if (!row) {
      if (unrecognized.length < 50) {
        unrecognized.push(unrecognizedItem(
          line,
          ticker,
          "A possible ticker was visible, but it could not be aligned with the statement value columns.",
        ));
      }
      return;
    }
    const rowEvidence = lines.slice(row.index, Math.min(lines.length, index + 1)).join("\n").slice(0, 1000);
    const name = fidelityNameFromRow(row.line, ticker);
    seen.add(ticker);
    holdings.push({
      ticker,
      name,
      shares: row.values[1],
      marketValue: row.values[3],
      percent: null,
      category: "Needs review",
      confidence: "low",
      confidenceReason: "The ticker, quantity, and ending market value align on a Fidelity position row; security identity still needs independent verification.",
      fieldConfidence: {
        ticker: "high",
      ...
      warnings: [warning(
        "unverified-security",
        `The statement reads ${ticker}, but its identity has not been independently verified yet.`,
        "Review the verification result before analyzing.",
      )],
      rowEvidence,
      verification: {
        status: "unresolved",
        evidence: [`Ticker ${ticker}, quantity, and ending market value were aligned from the same Fidelity position row.`],
      },
    });
  });
...
function parsePortfolioText(ocrText, totalPortfolioValue = 0, options = {}) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const candidate of candidates) {
      ...
      const structuredTickerRow = Boolean(
        name
        || /\([A-Z]{1,5}\)/.test(line)
        || new RegExp(`\\b${ticker.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s+[-$\\d]`).test(line),
      );
      const defensible = structuredTickerRow && (fields.marketValue !== null || fields.percent !== null);
      if (!defensible) {
        if (unrecognized.length < 50) {
          unrecognized.push(unrecognizedItem(
            line,
            ticker,
            "Required numeric position data was missing.",
          ));
        }
        continue;
      }
